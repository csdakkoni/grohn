-- Fix 'type' column in stock_movements for all RPC functions
...
BEGIN
    ...
    -- Log Movement (Added type='In')
    INSERT INTO stock_movements (
        user_id, inventory_id, type, change_amount, current_stock, reason, related_id, notes
    ) VALUES (
        p_user_id, v_inventory_id, 'In', p_qty, v_current_stock, 'Purchase', v_purchase_id, 'Satınalma: ' || p_lot_no
    );
...
BEGIN
    ...
    FOR v_ingredient IN SELECT * FROM recipe_ingredients WHERE recipe_id = p_recipe_id LOOP
        ...
        -- Log Usage (Added type='Out')
        SELECT COALESCE(SUM(qty), 0) - v_remaining_qty INTO v_current_stock FROM lots WHERE inventory_id = v_ingredient.item_id;
        INSERT INTO stock_movements (
            user_id, inventory_id, type, change_amount, current_stock, reason, related_id, notes
        ) VALUES (
            p_user_id, v_ingredient.item_id, 'Out', -v_remaining_qty, v_current_stock, 'Production_Usage', v_production_id, 'Üretim: ' || v_lot_number
        );
    ...
    -- Log Output (Added type='In')
    INSERT INTO stock_movements (
        user_id, inventory_id, type, change_amount, current_stock, reason, related_id, notes
    ) VALUES (
        p_user_id, v_product.id, 'In', p_quantity, v_current_stock, 'Production_Output', v_production_id, 'Üretim: ' || v_lot_number
    );
...
BEGIN
    ...
    FOR v_ingredient IN SELECT * FROM recipe_ingredients WHERE recipe_id = v_prod.recipe_id LOOP
        ...
        BEGIN
            ...
            INSERT INTO stock_movements (
                ...
                p_user_id, v_ingredient.item_id, 'Out', -v_remaining_qty, v_current_stock, 'Production_Usage', p_production_id, 'Üretim Tamamlama: ' || v_lot_number
            );
    ...
    -- Log Output Movement (Added type='In')
    SELECT COALESCE(SUM(qty), 0) INTO v_current_stock FROM lots WHERE inventory_id = v_product.id;
    INSERT INTO stock_movements (
        user_id, inventory_id, type, change_amount, current_stock, reason, related_id, notes
    ) VALUES (
        p_user_id, v_product.id, 'In', v_prod.quantity, v_current_stock, 'Production_Output', p_production_id, 'Üretim Çıktısı: ' || v_lot_number
    );
...
BEGIN
    ...
    -- Log Movement (Added type='Out')
    SELECT COALESCE(SUM(qty), 0) INTO v_current_stock FROM lots WHERE inventory_id = v_product_id;
    INSERT INTO stock_movements (
        user_id, inventory_id, type, change_amount, current_stock, reason, related_id, notes
    ) VALUES (
        p_user_id, v_product_id, 'Out', -p_quantity, v_current_stock, 'Sale', v_sale_id, 'Satış: ' || v_production.lot_number
    );
