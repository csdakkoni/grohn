...
BEGIN
    ...
    FOR v_ingredient IN SELECT * FROM recipe_ingredients WHERE recipe_id = p_recipe_id LOOP
        ...
        BEGIN
            ...
            v_raw_cost := v_raw_cost + (v_converted_cost * v_ingredient.percentage / 100 * p_quantity);
            v_pay_term_sum := v_pay_term_sum + (v_converted_cost * v_ingredient.percentage / 100 * p_quantity * COALESCE(v_item_term, 0));
            v_cost_sum := v_cost_sum + (v_converted_cost * v_ingredient.percentage / 100 * p_quantity);
    ...
    -- Split Financing Logic: Raw materials benefit from supplier terms, upfront costs (Pkg, Ship, Overhead) are paid at day 0
    v_financing_cost := (
        (v_raw_cost * (p_monthly_interest_rate / 100.0 / 30.0) * v_fin_days) +
        ((v_pkg_cost + (p_shipping_cost * p_quantity) + (p_overhead_cost * p_quantity)) * (p_monthly_interest_rate / 100.0 / 30.0) * p_sale_term_days)
    );

    -- Totals (Treat shipping and overhead as UNIT costs, multiply by quantity)
    v_total_cost := v_raw_cost + v_pkg_cost + (p_shipping_cost * p_quantity) + (p_overhead_cost * p_quantity) + v_financing_cost;
    ...
    INSERT INTO productions (
        ...
        v_raw_cost, v_pkg_cost, (p_shipping_cost * p_quantity), (p_overhead_cost * p_quantity), p_sale_term_days,
...
BEGIN
    ...
    -- Split Financing: Raw materials benefit from supplier lead time, upfront costs (Pkg, Ship, Overhead) are paid at day 0
    v_financing_cost := (
        (v_raw_cost * (p_monthly_interest_rate / 100.0 / 30.0) * v_fin_days) +
        ((v_pkg_cost + (p_shipping_cost * v_prod.quantity) + (p_overhead_cost * v_prod.quantity)) * (p_monthly_interest_rate / 100.0 / 30.0) * p_sale_term_days)
    );

    -- Totals
    v_total_cost := v_raw_cost + v_pkg_cost + (p_shipping_cost * v_prod.quantity) + (p_overhead_cost * v_prod.quantity) + v_financing_cost;
    ...
    UPDATE productions SET
        ...
        shipping_cost = (p_shipping_cost * v_prod.quantity),
        overhead_cost = (p_overhead_cost * v_prod.quantity),
    ...
    INSERT INTO quality_batches (
        ...
        p_production_id,
        'Production',
        'Otomatik oluşturuldu (Üretim Tamamlandı)',
        NOW()
    );
