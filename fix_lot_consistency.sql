-- FIX: Reuse existing Lot Number during production completion
...
BEGIN
    ...
    -- REUSE existing LOT if it exists (planned productions already have one)
    IF v_prod.lot_number IS NOT NULL AND v_prod.lot_number != '' THEN
        v_lot_number := v_prod.lot_number;
    ELSE
        -- Generate Real LOT Number only if missing
    ...
    -- AUTO-CREATE QC BATCH (if not existing or just update)
    -- Checking if a QC batch already exists for this production and lot
    -- Planned productions usually have a QC batch created when user clicks "Kalite Kontrole Gönder" 
    -- But let's check by production_id and lot_no
    IF EXISTS (SELECT 1 FROM quality_batches WHERE production_id = p_production_id AND lot_no = v_lot_number) THEN
        ...
        SET notes = notes || ' (Üretim Tamamlandı)'
