-- Add status and QC columns to productions table
ALTER TABLE productions ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'Completed'; -- 'Planned', 'Completed', 'Cancelled'
ALTER TABLE productions ADD COLUMN IF NOT EXISTS qc_status TEXT DEFAULT 'Pending'; -- 'Pending', 'Pass', 'Fail'
ALTER TABLE productions ADD COLUMN IF NOT EXISTS qc_notes TEXT;

-- RPC: Create Production Plan (No stock deduction yet)
...
BEGIN
    ...
    -- Generate Preliminary LOT (or placeholder)
    v_lot_number := 'PLAN-' || to_char(p_production_date, 'YYYYMMDD') || '-' || floor(random() * 1000)::text;

    -- Insert Production with 'Planned' status
...
-- RPC: Complete Production (Deduct stock, add product, update status)
